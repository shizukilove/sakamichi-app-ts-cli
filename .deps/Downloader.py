...
def downloader_mode_5(
                ...
                for asset in assets.objects:
                    ...
                    if asset.type.name == "Texture2D" and re.match(
                        ...
                        asset.read().image.save(
                            os.path.join(folder_path, f"{filename}.png")
                        )
    ...
    for assetBundle in datas:
        ...
        if is_downloaded(assetBundleName, lines):
            continue
...
def get_downloaded_map():
    download_log_file_name = f".catalog/{asset_type}_download_log.txt"
    with open(download_log_file_name, "r") as file:
        lines = file.read().splitlines()


def is_downloaded(name: str, list: list[str]):
    for line in list:
